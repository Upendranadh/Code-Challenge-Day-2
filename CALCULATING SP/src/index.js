...
function find_SellingPrice(a, b) {
  // loss=((costprice-sellingprice)/cp)*100
  // The above is the formuala for loss
  /*
    formula to calculate the sp
   sp=cp- (loss*cp)/100
   */
  let sellingprice;
  sellingprice = a - Math.round((b * a) / 100);
  if (sellingprice) return sellingprice;
